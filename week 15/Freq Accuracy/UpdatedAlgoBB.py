...
for i in range(len(Alldata)):
    ...
    for j in range(len(Alldata[i])):
        ...
        if '$' in Alldata[i][j][0]:
            Alldata[i][j][0] = Alldata[i][j][0].replace('$', '')
        if 'k' in Alldata[i][j][0]:
            Alldata[i][j][0] = Alldata[i][j][0].replace('k', '')
            temp = float(Alldata[i][j][0]) * 1000
            Alldata[i][j][0] = str(temp)
        if 'K' in Alldata[i][j][0]:
            Alldata[i][j][0] = Alldata[i][j][0].replace('K', '')
            temp = float(Alldata[i][j][0]) * 1000
            Alldata[i][j][0] = str(temp)
        if Alldata[i][j][3] in Threshold:
            temp = float(Alldata[i][j][0])
            # print(temp)
            if temp < Threshold[Alldata[i][j][3]]:
                BenignIdentifier[Alldata[i][j][3]] = 1
            else:
                BenignIdentifier[Alldata[i][j][3]] = 0
    if sum(BenignIdentifier.values()) / len(BenignIdentifier) >= 0.82:
        BenCount += 1
print('Benign Detected: ', (BenCount/len(Alldata))*100, '%')
